...
$pass=$_POST['pass'];
$e_mail=$_POST['email'];
$sql="SELECT * FROM $table WHERE user_name='$user' OR email='$e_mail'" ;
...
else
{
		...
		$mysqli="INSERT INTO $table (user_name,password,email) VALUES('$user','$pass','$e_mail')";
	if(!mysqli_query($conn,$mysqli))
    die(mysqli_error($conn));
	echo"</br><font color='green'><p>Signed up successfully</p></font>";
 ...
 <label class="col-sm-2 control-label">User name:</label>
      ...
      <div class="col-sm-10"> 
        <input class="form-control" name="pass" type="password" required >
	  	    ...
	  	    <button type="submit" name="submit1" class="btn btn-default" >Sign up</button>
...
function isAlphabet(){
var elem=document.forms["myForm"]["user"];
   var elen=document.getElementById("user").value;
...
return true;
		...
		alert("not possible:please make sure spaces and * are not present is user name");
		elem.focus();
		return false;
	}
}
...
if(	$GLOBALS['flag']===1)
{
	echo"</br><font color='red'><p>Sorry!!!user name or e-mail id already under use</p></font>";
}
